...
class HeroimageController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('logo_image')) {
            $image1 = $request->file('logo_image');
            $logo = microtime(true) . '-' . $image1->getClientOriginalExtension();
            $image1->move(public_path('images'), $logo);
        } else {
            $logo = null; // Set to null if no image is provided
        }
